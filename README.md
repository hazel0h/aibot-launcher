...
- Vercel/Supabase MCP 연결: Vercel은 Notion처럼 브라우저 OAuth, Supabase는 액세스 토큰 입력으로 바로 연결 (정보 수정 창의 "MCP 연결" 섹션)
...
- **Claude 계정/Notion 연결은 기본적으로 모든 에이전트가 공유합니다.** Claude Code는 로그인·MCP(Notion) 인증·플러그인을 컴퓨터 공용 설정 폴더(`~/.claude`)에 저장해서, 한 에이전트에서 다른 계정으로 로그인하면 전부 같이 바뀝니다. 에이전트별로 계정을 나누려면 **새 에이전트 만들기 / 정보 수정**의 **"이 에이전트 전용 Claude 계정 사용"** 을 켜세요. 그 에이전트만 `~/.claude-agents/<이름>` 폴더를 따로 써서(`CLAUDE_CONFIG_DIR`) 로그인·Notion 연결·플러그인이 완전히 분리됩니다. 켠 뒤 첫 세션 시작 때 Discord 플러그인이 자동 설치되고, 세션 터미널에서 그 에이전트용 계정으로 로그인하면 됩니다(Notion도 그 에이전트에서 다시 연결).
